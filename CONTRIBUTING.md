...
メジャーバージョンは mFLOCSS エコシステム（[mFLOCSS 仕様書](https://github.com/mflocss/spec) 等）と揃えます。そのため、**利用者のコードとの互換性が切れる変更でも、メジャーを上げずに CHANGELOG の該当項へ `**BREAKING**:` を付けて明示**します。
...
0-2. markuplint を上げたリリースでは、`markuplint.config.cjs` の `head-element-order` を既定値と突き合わせる（手順は [CODING_GUIDE.md「markuplint 設定のメンテナンス」](./CODING_GUIDE.md#markuplint-設定のメンテナンス)）
