...
> **前提**: 実行には **Node.js v24 以降**が必要です。未導入なら [nodejs.org](https://nodejs.org/) から LTS 版を入れてください（パッケージ管理コマンドの `npm` も同梱されます）。
...
2. 作成したリポジトリをクローン:
   ...
   npm run dev
   ```
   pnpm 派は `npm install` の代わりに `pnpm install`（同梱の `pnpm-lock.yaml` で再現ビルド。要 pnpm 10.5.1 以降。詳細は [CODING_GUIDE.md](./CODING_GUIDE.md#パッケージマネージャー)）。
